...
int taskCount = 0;  // variable n7sbo  bih bach nb9aw n testiw
...
void filterTasksByPriority() { // hada bach ydkhal user filter t9di blast styrin trdih boolean true false wtb9ay t9oli lo user dkhal 1 wla 2 mais haka bach man39dhach 3lik wnti 3ad badya
    char priority[10];
    printf("Enter priority to filter (High/Low): ");
    scanf(" %s", priority);

    printf("\nFiltered Tasks by Priority (%s):\n", priority);
    int found = 0;  // 7ta hna nfs chi n9dar nraj3O boolean mais 0 wla 1 rah tahowa 9adi lgharad
    ...
    if (!found) { // ila b9at found o donc atb9a false donc false * true hiya false walakin false f true hiya true wghadkhal had l3iba fkrini nchr7halk f audio
...
void sortTasksByDueDate() {   // hadi sort function wsfis used to arrange a collection of elements, such as an array or list, in a specific order.
    ...
    printf("Tasks sorted by due date in ascending order.\n");
    viewTasks();
}
...
int main() {
    ...
    do { // hana drt loop bach yb9a dima yswl machi ydor mra w7da wy7bas
        displayMenu();
        scanf("%d", &choice);
